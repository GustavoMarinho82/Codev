...
void ListaNosInternos(No * T, NoL * &R) {
	/* adiciona os nós internos de T na lista R */
	
	if (T != NULL) {
		if ((T->Esq != NULL) or (T->Dir != NULL)) {
			NoL * novo = (NoL *) malloc(sizeof(NoL));
			novo->E = T->E;
			novo->Prox = R;
			R = novo;
		}
		ListaNosInternos(T->Esq, R);
		ListaNosInternos(T->Dir, R);
	}
}

NoL * ListaNosInternos(No * T) {
	NoL * R = NULL;
	ListaNosInternos(T, R);
	return R;
}
